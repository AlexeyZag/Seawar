...
# обратите внимание на 32 строку, она позволяет выводить список скрытых кораблей ИИ (чит для отладки)
import random
from markT import MarkT
from remove_around import RemoveSq1
def play():
    global n, Q, L, Q1, allow_list_AI, allow_list_Pl, AI_count, player_count, countA, shipListP, move_list_AI, move_list_Pl
    global allow_list_Pl1, allow_list_AI1, shipListAI
    ...
    allow_list_Pl1 = set(([k for k in range(100)]))  # список клеток, куда можно ставить корабли на доску игрока, используются для расположение кораблей на досках
    ...
    place_all_ships()  # располагает все корабли
    ...
    try:
        ...
        if coordinate not in allow_list_AI:
            ...
            if Q1[coordinate] == " ":  # если на невидимой доске наш выстрел попадает на пустое поле, то печатаем Т на видимой
                ...
                remove_from_shiplist()  # убираем корабль ИИ из списка доступных, маркируем вокруг него Т
                paint()
        try:
...
def remove_from_shiplist():   # убирает корабль ИИ из списка доступных, маркирует вокруг него Т
    global Q, shipListAI, Q1, allow_list_AI
    for i in range(len(shipListAI)):
        if shipListAI[i] <= move_list_Pl:
            ...
            for k in w:
                remove_from_allowP = RemoveSq1(k, allow_list_AI)
                allow_list_AI.intersection_update(remove_from_allowP.remover1)
            shipListAI.pop(i)
            return
def remove_from_shiplistP():  # убирает корабль игрока из списка доступных, маркирует вокруг него Т
    global Q, shipListAI, Q1, L, move_list_AI, allow_list_Pl
    for i in range(len(shipListP)):
        if shipListP[i] <= move_list_AI:
            ...
            for k in w:
                remove_from_allowAI = RemoveSq1(k, allow_list_Pl)
                allow_list_Pl.intersection_update(remove_from_allowAI.remover1)
            shipListP.pop(i)
            return
...
def place1s(allow, listBoard, ship, shipMark):  # функция, устанавливающая 1-палубные корабли, в качестве аргументов принимает:
    ...
    ship.append({shoot})  # добавляем корабль в список игрока или ИИ
...
def place2s(allow, listBoard, ship, shipMark):  # функция, устанавливающая 1-палубные корабли
    ...
    if len(allow_place_2) != 0:  # если существуют пустые клетки возле первого вхождения, то выбираем рандомно второе поле
        ...
        listBoard[place1] = 'S'
        listBoard[place2] = 'S'
        ship.append({place1, place2})  # добавляем корабль в список кораблей
        ...
        place2s(allow, listBoard, ship, shipMark)
...
def place3s(allow, listBoard, ship, shipMark):
    ...
    if len(allow_place3_2) != 0:
        ...
        place3s(allow, listBoard, ship, shipMark)
    ship.append({place3_1, place3_2, place3_3})
...
def place4s(allow, listBoard, ship, shipMark):
    ...
    if len(allow_place4_2) != 0:
        ...
        if place4_2 == place4_1 + 1:
            ...
            place4_3 = place4_2 + 10
            place4_4 = place4_2 + 20
        ship.append({place4_1, place4_2, place4_3, place4_4})
        allow.remove(place4_1)
        allow.remove(place4_2)
        allow.remove(place4_3)
        allow.remove(place4_4)
    else:
        place4s(allow, listBoard, ship, shipMark)
...
def place_all_ships():  # устанавливает все корабли на доске игрока и на невидимой доске
    global L, Q1, shipListAI
    shipP4, shipP3_1, shipP3_2, shipP2_1, shipP2_2, shipP2_3, shipP1_1, shipP1_2, shipP1_3, shipP1_4 = set(), set(), set(), set(), set(), set(), set(), set(), set(), set()
    shipA4, shipA3_1, shipA3_2, shipA2_1, shipA2_2, shipA2_3, shipA1_1, shipA1_2, shipA1_3, shipA1_4 = set(), set(), set(), set(), set(), set(), set(), set(), set(), set()
    place4s(allow_list_Pl1, L, shipListP, shipP4)
    place3s(allow_list_Pl1, L, shipListP, shipP3_1)
    place3s(allow_list_Pl1, L, shipListP, shipP3_2)
    place2s(allow_list_Pl1, L, shipListP, shipP2_1)
    place2s(allow_list_Pl1, L, shipListP, shipP2_2)
    place2s(allow_list_Pl1, L, shipListP, shipP2_3)
    place1s(allow_list_Pl1, L, shipListP, shipP1_1)
    place1s(allow_list_Pl1, L, shipListP, shipP1_2)
    place1s(allow_list_Pl1, L, shipListP, shipP1_3)
    place1s(allow_list_Pl1, L, shipListP, shipP1_4)
    place4s(allow_list_AI1, Q1, shipListAI, shipA4)
    place3s(allow_list_AI1, Q1, shipListAI, shipA3_1)
    place3s(allow_list_AI1, Q1, shipListAI, shipA3_2)
    place2s(allow_list_AI1, Q1, shipListAI, shipA2_1)
    place2s(allow_list_AI1, Q1, shipListAI, shipA2_2)
    place2s(allow_list_AI1, Q1, shipListAI, shipA2_3)
    place1s(allow_list_AI1, Q1, shipListAI, shipA1_1)
    place1s(allow_list_AI1, Q1, shipListAI, shipA1_2)
    place1s(allow_list_AI1, Q1, shipListAI, shipA1_3)
    place1s(allow_list_AI1, Q1, shipListAI, shipA1_4)
play()
