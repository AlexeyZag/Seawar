...
import random
from remover2 import RemoveSq2
from remover3 import RemoveSq3
from remover4 import RemoveSq4
def play():
    global n, Q, L, Q1, allow_list_AI, allow_list_Pl
    ...
    while True:  # играем до победы
        if AI_count == 20:
            print('Выиграл бот')
            if input('хотите сыграть ещё раз? (y/n') == 'y'.lower():
                play()
            else:
                print('Выход')
                return
...
def input_nump():  # ввод хода за игрока
    global n, player_count, allow_list_AI, allow_list_Pl
    ...
    try:
        ...
        if coordinate not in allow_list_AI:
            ...
            if Q1[coordinate] == " ":  # если на невидимой доске наш выстрел попадает на пустое поле, то печатаем Т на пустой
                Q[coordinate] = 'T'
                n += 1
            elif Q1[coordinate] == 'S':  # если на невидимой доске наш выстрел попадает на пустое поле, то печатаем S на пустой
                Q[coordinate] = 'X'
                player_count += 1
                paint()
...
def place1s(allow, listBoard, ship):  # функция, устанавливающая 1-палубные корабли, в качестве аргументов принимает:
    ...
    try:
        ...
        pass
    try:
        allow.remove(shoot - 9)
    except KeyError:
        pass
    try:
        allow.remove(shoot - 10)
    except KeyError:
        pass
    try:
        allow.remove(shoot - 11)
    except KeyError:
        pass
    try:
        allow.remove(shoot + 9)
...
def place2s(allow, listBoard, ship):  # функция, устанавливающая 1-палубные корабли
    ...
    if len(allow_place_2) != 0:  # если существуют пустые клетки возле первого вхождения, то выбираем рандомно второе поле
        ...
        place2s(allow, listBoard, ship)
    allow_list_Pl1_1 = RemoveSq2(place1, place2, allow)  # создаем переменную вызова класса
    allow.intersection_update(allow_list_Pl1_1.remover)  # обращаемся к классу, который убирает рядом стоящие клетки из списка дозволенных
def place3s(allow, listBoard, ship):
    global place3_3, place3_1, allow_list_Pl1, allow_list_Pl1_2
    ...
    if len(allow_place3_2) != 0:
        ...
        place3s(allow, listBoard, ship)
    allow_list_Pl1_2 = RemoveSq3(place3_1, place3_3, allow)
    allow.intersection_update(allow_list_Pl1_2.remover3)
def place4s(allow, listBoard, ship):
    ...
    if len(allow_place4_2) != 0:
        ...
        place4s(allow, listBoard, ship)
    allow_list_Pl1_4 = RemoveSq4(place4_1, place4_4, allow)
    allow.intersection_update(allow_list_Pl1_4.remover4)
